...
async def test_story_agent_valid_json():
    ...
    valid_json = json.dumps({
        ...
        "beats": [
            {"beat_id": "b1", "title": "Intro", "summary": "Start", "emotional_arc": "Neutral", "setting": "Street"}
        ]
    })
    executor = MockJSONProvider(valid_json)
    ctx = AgentContext(task_id="t1", goal_id="g1", project_id="p1", target_agent_type="STORY", sections=[])
...
async def test_agent_schema_robustness_malformed_json():
    ...
    assert "JSON validation error" in result.error_message or "validation error" in result.error_message.lower()
...
async def test_agent_schema_robustness_invalid_schema():
    ...
    ctx = AgentContext(task_id="t1", goal_id="g1", project_id="p1", target_agent_type="CHARACTER", sections=[])
...
async def test_dialogue_agent_valid_json():
    ...
    valid_json = json.dumps({
        "scene_id": "s1",
        "bubbles": [
            {"bubble_id": "b1", "panel_number": 1, "character_id": "c1", "dialogue_type": "SPEECH", "text": "Hello"}
        ]
    })
    executor = MockJSONProvider(valid_json)
    ctx = AgentContext(task_id="t1", goal_id="g1", project_id="p1", target_agent_type="DIALOGUE", sections=[])
