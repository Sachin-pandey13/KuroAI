...
class PanelExportSlot(BaseModel):
    ...
    speech_bubbles: List[dict] = Field(
        default_factory=list, description="Overlay speech bubble text and styling specs"
    )
...
class PageExportBundle(BaseModel):
    ...
    panels: List[PanelExportSlot] = Field(
        default_factory=list, description="Ordered panel export slots"
    )
...
class ExportManifest(BaseModel):
    ...
    pages: List[PageExportBundle] = Field(
        default_factory=list, description="Pages compiled in export bundle"
    )
    export_format: str = Field(
        default="PDF_MANIFEST", description="Target export format (HTML, SVG, PDF)"
    )
