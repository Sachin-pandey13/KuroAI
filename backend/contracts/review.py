...
class ReviewIssue(BaseModel):
    ...
    description: str = Field(
        description="Human-readable detail of what went wrong or needs attention"
    )
    panel_number: Optional[int] = Field(
        default=None, description="Associated panel number if applicable"
    )
...
class ReviewFeedback(BaseModel):
    ...
    confidence: Optional[float] = Field(
        default=1.0, ge=0.0, le=1.0, description="Model confidence in evaluation"
    )
...
class ContinuityReport(BaseModel):
    ...
    issues: List[ReviewIssue] = Field(
        default_factory=list, description="Continuity discrepancies found across scenes"
    )
    characters_checked: List[str] = Field(
        default_factory=list, description="Character IDs checked for consistency"
    )
