...
class HumanReviewGate(BaseModel):
    ...
    action: Optional[HumanReviewAction] = Field(
        default=None, description="Human decision (None if pending)"
    )
    feedback_notes: Optional[str] = Field(
        default=None, description="Human feedback / instructions for revision"
    )
    reviewed_at: Optional[datetime] = Field(
        default=None, description="Timestamp when review action was recorded"
    )
