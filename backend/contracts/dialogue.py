...
class SpeechBubble(BaseModel):
    ...
    character_id: Optional[str] = Field(description="ID of the character speaking, None if Narration")
    dialogue_type: DialogueType = Field(description="Type of bubble (Speech, Thought, etc.)")
    text: str = Field(description="The actual text content")
    emotion_tag: Optional[str] = Field(description="Underlying emotion driving this dialogue line (e.g., Angry, Sad, Neutral)", default=None)

class SceneDialogue(BaseModel):
    scene_id: str = Field(description="The scene these speech bubbles belong to")
    bubbles: List[SpeechBubble] = Field(description="All speech bubbles in the scene, ordered chronologically", default_factory=list)
