...
from backend.telemetry.metrics import Counter, EventAuditRegistry, Histogram
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, request, *args, **kwargs):
            with tracer.start_as_current_span(
                ...
                {
                    "capability_type": getattr(request, "capability_type", ""),
                ...
                try:
                    ...
                    if hasattr(response, "success") and not response.success:
                        span.set_status(
                            "ERROR", getattr(response, "error_message", "Execution failed")
                        )
                    else:
                        span.set_status("OK")
                    return response
...
def instrument_context_assembly(tracer: Tracer) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(self, task, *args, **kwargs):
            with tracer.start_as_current_span(
                "ContextEngine.assemble_context", {"task_id": getattr(task, "task_id", "")}
            ) as span:
