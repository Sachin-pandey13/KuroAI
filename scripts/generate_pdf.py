...
def create_kuroai_pdf(output_path):
    ...
    desc = (
        "An enterprise-grade, AI-driven storytelling platform serving as a "
    ...
    write_bullet(
        "AI Storytelling:",
        "Seamlessly map user-defined creative premises to multi-panel manga storyboard layouts.",
    )
    write_bullet(
        "Visual Consistency:",
        "Enforce strict character feature details (hair style, eyes, outfits) and artistic linework style across multiple generated frames.",
    )
    write_bullet(
        "Service Decoupling:",
        "Establish isolated API service zones, ensuring frontend presentation, backend business logic, and heavy compute AI pipelines are decoupled.",
    )
    write_bullet(
        "Extensibility:",
        "Design clean adapters for LLMs (Ollama/Mistral) and diffusion systems, facilitating future vendor swaps or model upgrades.",
    )
    ...
    write_bullet(
        "React & TS Client:",
        "Modern frontend designed with Vite. Implements Framer Motion and custom CSS variables for premium visual responsiveness and interactive canvas tools.",
    )
    write_bullet(
        "Spring Boot Backend:",
        "Serves as the gateway layer. Implements stateless JWT authentication, secures endpoints, and proxies generation payload calls to the python AI microservice.",
    )
    write_bullet(
        "FastAPI Service:",
        "Dedicated Python service running the AI orchestration engine. Integrates Hugging Face's Diffusers pipeline and local LLM clients.",
    )
    ...
    write_bullet(
        ...
        "Implement automated testing and formatting checks via GitHub Actions, combined with static analysis security testing (SAST) to secure code layers.",
    )

    write_bullet(
        "Event-Driven AI Queue (Kafka):",
        "Decouple HTTP POST requests from model run cycles. Using Kafka or RabbitMQ, jobs are queued instantly and executed asynchronously, improving throughput.",
    )

    write_bullet(
        "Distributed Caching (Redis):",
        "Cache popular model prompts and images to dramatically lower GPU computation expenses and page load delays.",
    )

    write_bullet(
        "Metadata Versioning Control:",
        "A specialized Git-like versioning backend allowing creators to branch, edit, and rollback panel manipulations and dialogue lines securely.",
    )

    write_bullet(
        "Federated Single Sign-On (SSO):",
        "Incorporate Spring Security OAuth2 configurations to handle Google, GitHub, and Enterprise Active Directory identity integrations.",
    )
    ...
    pdf.cell(
        0,
        5,
        "Engineered to demonstrate modern AI/ML pipelines and decoupled microservices.",
        0,
        1,
        "L",
    )
